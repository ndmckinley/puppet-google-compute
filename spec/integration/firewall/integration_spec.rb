...
describe 'firewall.create', vcr: true do
  mods = [File.expand_path('.'), File.expand_path('./spec/fixtures/modules/')]
  it 'creates and destroys non-existent firewall' do
    puts 'pre-destroying firewall'
    VCR.use_cassette('pre_destroy_firewall') do
      ...
      Puppet.override(current_environment: test) do
        ...
        puppet.stubs(:command_line).returns(
          stub('command_line', args: [write_example('delete_firewall')])
        )
        begin
          puppet.run_command
        rescue SystemExit => e
    ...
    VCR.use_cassette('create_firewall') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
        puppet = Puppet::Application[:apply]
        puppet.stubs(:command_line).returns(
          stub('command_line', args: [write_example('firewall')])
        )
        begin
          puppet.run_command
        rescue SystemExit => e
    ...
    puts 'checking that firewall is created'
    VCR.use_cassette('check_firewall') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
        puppet = Puppet::Application[:apply]
    ...
    VCR.use_cassette('destroy_firewall') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
        puppet = Puppet::Application[:apply]
    ...
    puts 'checking that firewall is destroyed'
    VCR.use_cassette('check_firewall') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
