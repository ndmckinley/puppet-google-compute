...
describe 'disk.create', vcr: true do
  mods = [File.expand_path('.'), File.expand_path('./spec/fixtures/modules/')]
  it 'creates and destroys non-existent disk' do
    puts 'pre-destroying disk'
    VCR.use_cassette('pre_destroy_disk') do
      ...
      Puppet.override(current_environment: test) do
        ...
        puppet.stubs(:command_line).returns(
          stub('command_line', args: [write_example('delete_disk')])
        )
        begin
          puppet.run_command
        rescue SystemExit => e
    ...
    puts 'creating disk'
    VCR.use_cassette('create_disk') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
        puppet = Puppet::Application[:apply]
        puppet.stubs(:command_line).returns(
          stub('command_line', args: [write_example('disk')])
        )
        begin
          puppet.run_command
        rescue SystemExit => e
    ...
    puts 'checking that disk is created'
    VCR.use_cassette('check_disk') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
        puppet = Puppet::Application[:apply]
    ...
    puts 'destroying disk'
    VCR.use_cassette('destroy_disk') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
        puppet = Puppet::Application[:apply]
    ...
    puts 'checking that disk is destroyed'
    VCR.use_cassette('check_disk') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
