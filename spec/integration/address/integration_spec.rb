...
describe 'address.create', vcr: true do
  mods = [File.expand_path('.'), File.expand_path('./spec/fixtures/modules/')]
  it 'creates and destroys non-existent address' do
    puts 'pre-destroying address'
    VCR.use_cassette('pre_destroy_address') do
      ...
      Puppet.override(current_environment: test) do
        ...
        puppet.stubs(:command_line).returns(
          stub('command_line', args: [write_example('delete_address')])
        )
        begin
          puppet.run_command
        rescue SystemExit => e
    ...
    VCR.use_cassette('create_address') do
      ...
      Puppet.override(current_environment: test) do
        ...
        puppet = Puppet::Application[:apply]
        puppet.stubs(:command_line).returns(
          stub('command_line', args: [write_example('address')])
        )
        begin
          puppet.run_command
        rescue SystemExit => e
    ...
    puts 'checking that address is created'
    VCR.use_cassette('check_address') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
        puppet = Puppet::Application[:apply]
    ...
    puts 'destroying address'
    VCR.use_cassette('destroy_address') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
        puppet = Puppet::Application[:apply]
    ...
    puts 'checking that address is destroyed'
    VCR.use_cassette('check_address') do
      test = Puppet::Node::Environment.create(:test, mods)
      Puppet.override(current_environment: test) do
        Puppet[:environment] = 'special'
